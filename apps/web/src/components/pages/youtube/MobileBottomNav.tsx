...
import { Clock3, ListVideo, Mic2, MoreHorizontal, Search, Settings, SlidersVertical } from 'lucide-react';
...
const PRIMARY_TABS = ['search', 'songRequest', 'queue', 'controls'] as const;
    ...
    function MobileBottomNav({ className }, ref) {
        ...
        return (
            <nav
                ref={ref}
                className={cn(
                    'grid grid-cols-5 items-stretch border-t bg-background px-page-gutter pb-safe-offset pt-1',
                    className,
                )}
            ...
            >
                ...
                />
                <NavItem
                    active={activePrimary === 'songRequest'}
                    icon={<Mic2 className="h-6 w-6" />}
                    label={t('requestSong')}
                    onClick={() => setCurrentTab('songRequest')}
                />
                <NavItem
